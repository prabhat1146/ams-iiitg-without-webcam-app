...
const VerifyAndSetPasswordForFaculty = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true)
    const url = `${BASEURL}/verification/verifyEmailForFaculty?token=${token}`
    fetch(url)
      .then((res) => {
        if (res.ok) {
            return res.json()
      ...
      .then((res) => {
        ...
        if (res?.isEmailVerified) {
          ...
          console.log('r', data)
          const url = `${BASEURL}/verification/resetPasswordForFaculty`
          fetchData(url, data)
            .then((res) => {
              setIsLoading(false);
              if (res) {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-r from-blue-500 to-purple-500 ">
      ...
      <h2 className="text-2xl text-white p-8 font-semibold pt-32 mx-auto">Reset password or verify email for faculty</h2>
...
export default VerifyAndSetPasswordForFaculty;
